...
int main(int argc, char *argv[]) {
    ...
    if (inpath) {
        ...
        printf("compression time %ld , %ld, %ld\n", timeElapsed, CLOCKS_PER_SEC, timeElapsed/ (CLOCKS_PER_SEC/1000));
        long timeInMs = timeElapsed/ (CLOCKS_PER_SEC/1000);


        /* optionally write compressed data */
        if (zfppath) {
            char str[200];
            sprintf(str, "%s_%ld", zfppath,timeInMs);

            FILE *file = !strcmp(str, "-") ? stdout : fopen(str, "wb");
            if (!file) {
    ...
    if (!quiet) {
        ...
        fprintf(stderr, " raw=%lu zfp=%lu ratio=%.3g rate=%.4g", (unsigned long) rawsize, (unsigned long) zfpsize,
                (double) rawsize / zfpsize, CHAR_BIT * (double) zfpsize / (nx * ny * nz));
        if (stats)
