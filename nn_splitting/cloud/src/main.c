...
void draw_detections_cpu(image im, int num, float thresh, box *boxes, float **probs, char **names, image **alphabet, int classes, char *layer_data)
{
	int i;
	...
	for (i = 0; i < num; ++i) {
		...
		if (prob > thresh) {	// if (probability > threshold) the draw bonded box
			...
			//printf("%s; %s: %.0f;x_center = %d; y_center = %d; width = %d; height = %d \n",layer_data,
			//	names[class], prob * 100, (int)(b.x*im.w), (int)(b.y*im.h), (int)(b.w*im.w), (int)(b.h*im.h));
			printf("{class: '%s'},",  names[class]);
			if (left < 0) left = 0;
...
void test_detector_cpu(char **names, char *cfgfile, char *weightfile, char *filename, float thresh, char *layer_data, int startLayer)
{
   ...
   // printf("ssada");
	...
	char *layerData = folder_buff;
	int j;
	float nms = .4;
	while (1) {
		if (filename) {
			strncpy(input, filename, 256);
			strncpy(layerData, layer_data, 256);
		}
		else {
		...
		net.filename = layerData;
        net.startLayer = startLayer;
		...
		draw_detections_cpu(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes,layer_data );	// draw_detections(): image.c
...
void run_detector(int argc, char **argv)
{
	...
	int startLayer = (argc > 8) ?  atoi(argv[8]) : 0;
  ...
  //  printf("sda");
	...
	if (0 == strcmp(argv[2], "test")) test_detector_cpu(names, cfg, weights, filename, thresh, foldername, startLayer);
