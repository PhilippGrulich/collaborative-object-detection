...
void yolov2_forward_network_cpu(network net, network_state state) {
    ...
    FILE *filePtr;
    layer l = net.layers[net.startLayer + 1];
    filePtr = fopen(net.filename, "r");
    //printf("load %d floats\n", l.inputs);

    float *buffer = (float *) malloc(sizeof(float) * l.inputs);
    fread(buffer, 1,  l.inputs * sizeof(float), filePtr);
    state.input = buffer;

    //printf("first %f floats\n",buffer[0]);
    //printf("last %f floats\n", buffer[l.inputs-1]);
    for (i = net.startLayer + 1; i < net.n; ++i) {
        state.index = i;
        ...
        //printf("%d;%ld\n", l.outputs, timeElapsed / (CLOCKS_PER_SEC / 1000));
        if (i == net.startLayer) {
            printf("%f, %f, %f, %f,  ", buffer[0], state.input[0], buffer[l.outputs - 1],
