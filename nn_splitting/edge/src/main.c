...
void test_detector_cpu(char **names, char *cfgfile, char *weightfile, char *filename, float thresh, char *foldername, int maxLayer)
{
	//image **alphabet = load_alphabet();			// image.c
	image **alphabet = NULL;
	network net = parse_network_cfg(cfgfile);
	net.maxLayer = maxLayer;
	// parser.c
	if (weightfile) {
	...
	while (1) {
       ...
       // sprintf(path, "./data/%s/", net.filename);
		...
		printf("time: %f,", ((float)(clock() - time) / CLOCKS_PER_SEC)); //sec(clock() - time));
...
void run_detector(int argc, char **argv)
{
	...
	int maxLayer = (argc > 8) ? atoi(argv[8]) : 0;;
	...
	if (0 == strcmp(argv[2], "part")) {
		//printf("run partition till %d", maxLayer);
		test_detector_cpu(names, cfg, weights, filename, thresh, foldername, maxLayer);
	}

	if (0 == strcmp(argv[2], "test")) test_detector_cpu(names, cfg, weights, filename, thresh, foldername, -1);
